...
static int32_t bt_reg_init(void)
{
	...
	if (node) {
		for (i = 0; i < CONSYS_BASE_ADDR_MAX; i++) {
			...
			BTMTK_DBG("Get Index(%d) phy(0x%lx) baseAddr=(0x%lx) size=(0x%llx)",
				i, base_addr->phy_addr, base_addr->vir_addr, base_addr->size);
		}
				...
				*self, unsigned long event, void *data)
{
	...
	case FB_BLANK_POWERDOWN:
		if(g_bdev->bt_state == FUNC_ON) {
			...
			BTMTK_INFO("%s: blank state [%d]->[%d], and send cmd", __func__, g_bdev->blank_state, blank);
			g_bdev->blank_state = blank;
			btmtk_send_blank_status_cmd(g_bdev->hdev, blank);
		} else {
			BTMTK_INFO("%s: blank state [%d]->[%d]", __func__, g_bdev->blank_state, blank);
			g_bdev->blank_state = blank;
		}
		break;
	default:
...
static int btmtk_btif_probe(struct platform_device *pdev)
{
	...
	if (bt_reg_init()) {
		BTMTK_ERR("Error allocating memory remap");
		return -1;
	}
